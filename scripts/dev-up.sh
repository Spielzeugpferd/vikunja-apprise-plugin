#!/usr/bin/env bash
# Local workbench for this plugin: brings up Apprise API, wires this repo into a
# sibling Vikunja checkout, and runs Vikunja with the plugin loaded.
#
...
echo "==> Starting local Apprise API (docker compose)…"
(cd "$PLUGIN_DIR" && docker compose up -d)

echo "==> Wiring the plugin into $VIKUNJA_DIR/plugins/vikunja-apprise …"
mkdir -p "$VIKUNJA_DIR/plugins/vikunja-apprise"
ln -sf "$PLUGIN_DIR/main.go" "$VIKUNJA_DIR/plugins/vikunja-apprise/main.go"
...
exec env \
	...
	VIKUNJA_PLUGINS_APPRISE_APIURL=http://localhost:8000 \
